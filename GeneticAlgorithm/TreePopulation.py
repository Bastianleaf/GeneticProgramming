...
class TreePopulation:
	...
	def __init__(self, population, terminals, functions, generation_limit, solution):
		"""
	...
	def evaluate_fitness(self):
		...
		for chromosome in self.population:
			...
			if chromosome.score < self.best_score:
				self.best = TreeChromosome(chromosome.value.copy_tree())
				self.best.evaluate_fitness(self.solution)
	...
	def reproduction(self, pool):
		...
		for i in range(0, len(pool), 2):
			new_pool.append(pool[i].reproduction(pool[i + 1], self.solution, self.functions))
